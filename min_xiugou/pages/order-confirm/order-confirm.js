...
Page({
    ...
    payBtnClicked(){ // 提交订单
        ...
        API[reqUrl](params).then((res) => {
            let datas = res.data || {}
            let userAddress = datas.userAddressDTO || datas.userAddress || {}
            Tool.sensors('SubmitOrder', {
                orderID: datas.orderNo,
                orderAmount: datas.payAmount,
                receiverName: userAddress.receiver,
                receiverProvince: userAddress.province,
                receiverCity: userAddress.city,
                receiverArea: userAddress.area,
                receiverAddress: userAddress.address,
                transportationCosts: datas.totalFreightFee,
                couponsName: this.data.coupon.name,
                couponsAmount: this.data.coupon.value || 0,
                ifUseYiYuan: this.data.params.tokenCoin ? true : false,
                yiYuanCouponsAmount: this.data.params.tokenCoin
            })
            Event.emit('getLevel')
